...
import { getApi } from '../redux/cities/cities';

const Filter = () => {
  /* eslint-disable prefer-destructuring */

  const citiess = useSelector((state) => state.cityWeathers);
  ...
  useEffect(() => {
    if (citiess.length === 0) {
      const getApiData = async () => {
        ...
        const citiesL = await Promise.all(listOfCities.map(async (city) => {
  ...
  return (
    <div className="filter">
      <h1 className="filter-tit"> Filter</h1>
    </div>
  );
};
